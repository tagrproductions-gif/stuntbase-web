...
CREATE TABLE profiles_backup_$(date +%Y%m%d) AS 
...
UPDATE profiles SET website = COALESCE(website, website_url) WHERE website IS NULL;
...
UNION ALL
SELECT 'profiles_backup_$(date +%Y%m%d)' as table_name, COUNT(*) as row_count FROM profiles_backup_$(date +%Y%m%d);
